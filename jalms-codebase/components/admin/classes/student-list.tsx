...
import { removeStudent } from "@/lib/actions/enrollment.actions"
...
interface StudentListProps {
    classId: string
    students: (User & { enrollmentId: string })[]
}

export function StudentList({ classId, students }: StudentListProps) {
    const [studentToDelete, setStudentToDelete] = useState<string | null>(null)
    const [isDeleting, setIsDeleting] = useState(false)

    async function confirmDelete() {
        console.log("[StudentList] Confirm delete for:", studentToDelete)
        if (!studentToDelete) return

        setIsDeleting(true)
        const result = await removeStudent(classId, studentToDelete)
        console.log("[StudentList] Delete result:", result)
        setIsDeleting(false)
