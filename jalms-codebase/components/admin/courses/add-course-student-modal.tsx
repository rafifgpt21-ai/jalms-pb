...
import { getAvailableStudentsForCourse, enrollStudentToCourse } from "@/lib/actions/enrollment.actions"
...
interface AddCourseStudentModalProps {
    courseId: string
}

export function AddCourseStudentModal({ courseId }: AddCourseStudentModalProps) {
    ...
    useEffect(() => {
        async function fetchStudents() {
            setIsLoading(true)
            const result = await getAvailableStudentsForCourse(courseId, debouncedSearch)
            if (result.students) {
    ...
    }, [debouncedSearch, courseId, open])

    async function handleEnroll() {
        if (!selectedStudentId) return

        setIsEnrolling(true)
        const result = await enrollStudentToCourse(courseId, selectedStudentId)
