...
migrate((app) => {
  Object.keys(fieldsByCollection).forEach((collectionName) => {
    ...
    fieldsByCollection[collectionName].forEach((fieldName) => {
      ...
      if (!field || field.type !== "relation" || field.maxSelect === 99) return;
      collection.fields.removeByName(fieldName);
      collection.fields.add(new RelationField({ id: field.id, name: field.name, collectionId: field.collectionId, maxSelect: 99, cascadeDelete: field.cascadeDelete }));
    });
    app.save(collection);
  });
}, (app) => {});
